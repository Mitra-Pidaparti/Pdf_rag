...
# -------------------------
# Enhanced OpenAI Sentence Extraction with Keywords
# -------------------------
def extract_relevant_sentences_with_openai_and_keywords(query, context_text, keywords_list):
    """Extract all relevant sentences from context using OpenAI with keyword guidance."""
    
    keywords_str = ", ".join(keywords_list) if keywords_list else "No specific keywords provided"
    
    system_prompt = f"""You are an expert text analyzer. Extract content that could help answer the user's query, with special attention to the provided keywords.
...
- Contains ANY of the specified keywords: {keywords_str}
- Relates to the themes represented by the keywords
...
2. Pay special attention to content containing the specified keywords
3. Only return "NONE" if the text is completely unrelated to both the query topic AND the keywords
4. When in doubt, extract the content - it's better to include potentially relevant information
...
def retrieve_initial_pool_with_keywords(query, keywords_list, collection_name, pool_size=INITIAL_POOL_SIZE):
    ...
    combined_scores = (0.4 * bm25_scores_norm + 
                      0.3 * tfidf_scores_norm + 
                      0.3 * keyword_scores_norm)
...
def dense_rerank_candidates(query, candidates, rerank_size=DENSE_RERANK_SIZE):
    ...
    for i, candidate in enumerate(candidates):
        ...
        candidate["final_score"] = (0.2 * candidate["combined_score"] + 
...
def enhance_with_context_features_and_keywords(chunks, query, keywords_list):
    ...
    for chunk in chunks:
        ...
        chunk["enhanced_score"] = (
            ...
            (enhanced_keyword_score * 0.4)  # Keyword boost weight
        )
...
def optimized_hybrid_search_with_keywords(query, keywords_list, collection_name):
    ...
    3. Enhanced context features with heading boost and keyword matching
    4. Keyword-aware sentence extraction
    ...
    for i, candidate in enumerate(enhanced_candidates):
        ...
        extracted_sentences = extract_relevant_sentences_with_openai_and_keywords(
            query, candidate["chunk"], keywords_list
        )
...
if __name__ == "__main__":
    ...
    csv_filename = "Novartis_extractionQv2.csv"
    csv_headers = [

...
def llm_pre_final_layer(query, results, top=30):
    ...
    for result in results:
        prompt = (
            ...
            "Score the candidate's relevance to the query on a scale from 0 (not relevant) to 1 (highly relevant). "
            "Respond with only the score as a float."
            "Also if the sentence is not meaningful by itself give it a big penalty in the score"
        )
...
if __name__ == "__main__":
    ...
    for query in queries:
        ...
        context_file_exists = os.path.isfile("extracted_contexts9.csv")
        with open("extracted_contexts9.csv", mode="a", newline="") as context_file:

...
# -------------------------
# Gemma Sentence Extraction
# -------------------------
def extract_relevant_sentences_with_gemma(query, context_text, model_name="gemma3:27b"):
    """
    Use Gemma via Ollama API to extract all relevant sentences from context that answer the query.
    Returns a list of verbatim sentences from the context.
    """
    url = "http://10.101.240.7/ollama/api/generate"
...
Extract all relevant sentences:"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if response_content == "NO_RELEVANT_SENTENCES":
                return []
            sentences = [sent.strip() for sent in response_content.split(" | ") if sent.strip()]
            return sentences
        else:
            print(f"[ERROR] Gemma API call failed: {response.status_code}")
            return []
    except Exception as e:
        print(f"[ERROR] Gemma API call failed: {e}")
        return []
...
# -------------------------
# Enhanced Semantic Search with gemma Extraction
# -------------------------
def enhanced_semantic_search(query, collection, query_embedding, top_k=10):
    """Perform enhanced semantic search with Gemma sentence extraction."""
    # Initial retrieval
    ...
    # Create results with MMR ranking and Gemma extraction
    semantic_results = []
    for idx in mmr_indices:
        ...
        print(f"[INFO] Extracting sentences from chunk using Gemma for query: {query}...")
        extracted_sentences = extract_relevant_sentences_with_gemma(query,chunk)
        ...
        for sentence in extracted_sentences:
            ...
            semantic_results.append({
                ...
                "source": "Dense Embeddings + Gemma Extraction"
            })
    
    return semantic_results

# -------------------------
# Process Results with Gemma            
# -------------------------
def process_results_with_gemma(results, query, method_name="Gemmato"):
    """Process retrieved results using Gemmato extract relevant sentences."""
    processed_results = []
    ...
    for result in results:
        ...
        print(f"[INFO] Extracting sentences usingGemma for method_name: {method_name} and query: {query}...")
        extracted_sentences = extract_relevant_sentences_with_gemma(query, context)
        ...
        for sentence in extracted_sentences:
            ...
            processed_result["source"] = f"{method_name} + Gemma Extraction"
            processed_results.append(processed_result)
    
    return processed_results

# -------------------------
# Improved Hybrid Search with Gemma Integration
# -------------------------
def improved_hybrid_search(query, collection_name, top_k=5, context_range=3):
    ...
    print(f"[INFO] Starting hybrid search with Gemma  extraction for query: {query}")
    ...
    print("[INFO] Processing BM25 results with Gemma  extraction...")
    for i, idx in enumerate(top_bm25):
        ...
        for sentence in extracted_sentences:
            all_results["bm25"].append({
                ...
                "source": "BM25 + Gemma Extraction"
            })
    
    # Process TF-IDF results with Gemma extraction
    print("[INFO] Processing TF-IDF results with Gemma extraction...")
    for i, idx in enumerate(top_tfidf):
        ...
        # Use Gemma to extract relevant sentences
        extracted_sentences = extract_relevant_sentences_with_gemma(query, chunk)
        ...
        for sentence in extracted_sentences:
            all_results["tfidf"].append({
                ...
                "source": "TF-IDF + Gemma Extraction"
            })

    # Dense results already processed with Gemma extraction
    ...
    for result in fused_results:
        # Create a simplified representation of the matched sentence
        sentence_simplified = ' '.join(re.findall(r'\b\w+\b', result["matched_sentence"].lower()))
        sentence_hash = hash(sentence_simplified)
...
def llm_pre_final_layer(query, results, top=30):
    """
    Rerank results that already have Gemma extracted sentences.
    Since sentences are already extracted by Gemmajust need to score them.
    """
    ...
    for result in results:
        # Sentences are already extracted by Gema so we just score them
        prompt = (
            f"Query: {query}\n"
...
if __name__ == "__main__":
    ...
    for query in queries:
        ...
        print("\nTop Results (Gemma Extracted + Ranked by Relevance):")
        context_file_exists = os.path.isfile("extracted_contexts_gemma.csv")
        with open("extracted_contexts_gemma.csv", mode="a", newline="") as context_file:
            ...
            for i, result in enumerate(results_final):
                print(f"{i + 1}. Source: {result.get('source', 'N/A')} - Score: {result.get('llm_score', 0):.4f}")
                print(f"   Gemma Extracted Sentence: {result['matched_sentence']}")
    ...
    print(f"Processed all queries with Gemma sentence extraction")
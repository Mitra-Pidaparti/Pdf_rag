...
# Configuration
# -------------------------
INITIAL_POOL_SIZE =  100 # Large pool from BM25/TF-IDF
DENSE_RERANK_SIZE = 60   # Top candidates after dense reranking
...
def enhance_with_context_features(chunks, query):
    """Add context-aware features to enhance ranking."""
    print("[INFO] Adding context-aware features...")
    ...
    for chunk in chunks:
        ...
        chunk["enhanced_score"] = chunk["final_score"] + context_bonus
...
def optimized_hybrid_search_with_dense_reranking(query, collection_name):
    ...
    1. Retrieve large pool (1000) using BM25 + TF-IDF fusion
    ...
    """
    print(f"\n[INFO] Starting optimized hybrid search for query: {query}")
...
if __name__ == "__main__":
    ...
    # Initialize CSV file
    csv_filename = "optimized_hybrid_search_results.csv"
    csv_headers = [
        "User Query", "Extracted Sentences", "Chunk Context", "Page", 
        "Document", "BM25 Score", "TF-IDF Score", "Combined Lexical Score",
        "Semantic Score", "Final Score", "Context Score", "Enhanced Score",
        "Sentence Similarity", "Ultimate Score"
    ]
